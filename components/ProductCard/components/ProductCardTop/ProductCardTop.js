import Count from '../../../Count/Count';

export default {
  name: 'ProductCardTop',
  props: {
    onlyCart: { type: Boolean, default: false }, // карточка для корзины
    productInCart: { type: Boolean, default: false }, // товар в корзине
    countProductFromCart: { type: Number, default: -1 }, // кол-во в корзине
    btnCartAction: { type: Function, default: () => null },
    changeCountFromCart: { type: Function, default: () => null },
    productInComparison: { type: Boolean, default: false }, // товар в сравнение
    btnComparisonAction: { type: Function, default: () => null },
    getTitle: { type: Function, default: () => 'undefined' },
    getLink: { type: String, default: '/undefined' },
    name: { type: String, default: '' },
    image: { type: String, default: 'data:image/png;base64,' }
  },
  components: {
    Count
  },
  render (h) {
    const self = this
    const {onlyCart, productInCart, countProductFromCart, btnCartAction, changeCountFromCart, productInComparison, btnComparisonAction, getTitle, getLink, name, image} = self.$props
    // Управление кол-ом
    const actionBtnCart = productInCart ? h('label', { class: 'product-top-action product-top-cart' }, [
      h('button', { class: 'sts-icon sts-iconshopping-cart', attrs: { title: getTitle('cart-remove-product') }, on: { click: btnCartAction } }),
      h(Count, { props: { moreClass: 'small', callback: changeCountFromCart, startValue: countProductFromCart } })
    ]) : null
    // Управление сравнением
    const actionBtnComparison = productInComparison && !onlyCart ? h('label', { class: 'product-top-action product-top-comparison' }, [
      h('button', { class: 'sts-icon sts-iconchart-bar', attrs: { title: getTitle('comparison-remove-product') }, on: { click: btnComparisonAction } })
    ]) : null
    // Кнопки активности
    const actionsBtn = (productInCart || onlyCart) || productInComparison ? h('div', { class: 'product-top-actions' }, [
      actionBtnCart,
      actionBtnComparison
    ]) : null
    // Фото товара
    const productImage = h('router-link', { class: 'product-top-image', props: { to: { path: getLink } } }, [
      h('img', { attrs: { alt: name, src: image } })
    ])
    // Товар
    const productTitle = h('router-link', {
      props: { to: { path: getLink } },
      class: {'product-top-title': true, 'has-cart': productInCart, 'has-comparison': productInComparison}
    }, name)
    // render
    return h('div', { class: 'product-top' }, [actionsBtn, productImage, productTitle])
  }
}
