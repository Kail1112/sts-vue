...
import {spaceBetweenNumber} from '~/plugins/filters.js';

import LoadingIndicator from '../LoadingIndicator/LoadingIndicator';
import ProductCardTop from './components/ProductCardTop/ProductCardTop';
import ProductCardMiddle from './components/ProductCardMiddle/ProductCardMiddle';
import ProductCardBottom from './components/ProductCardBottom/ProductCardBottom';
...
export default {
  ...
  methods: {
    ...
    actionForBtn (method = '', param = {id = -1, count = 1} = {id: -1, count: 1}) {
      const {id} = param // Получаем значенияs
      const {$store: store} = this.$root
      ...
      switch (method) {
        case 'add-cart' : store.dispatch('changeCart', {...param, callback}); break;
    ...
    changeCountFromCart (value, useTimeOut = true) {
      const funcSendRequest = (id = -1, count = -1) => {
        ...
        if ((Number.isInteger(clearCount) && clearCount > -1) &&
          ...
          templateProduct[id] = clearId
          templateProduct[count] = clearCount
    ...
    btnCartAction () {
      ...
      if (this.$root.$store.getters.CHECK_PRODUCT_IN_CART(clearId))
        ...
        this.actionForBtn('add-cart', templateProduct)
      }
    },
    /*----------------------*/
  ...
  render (h) {
    ...
    const objPropsForBottom = {
      price: self.$props.price,
      oldPrice: self.$props.oldPrice,
      spaceBetweenNumber,
      getLink: self.getLink,
      getTitle: self.getTitle,
      productInComparison: self.productInComparison,
      btnComparisonAction: self.btnComparisonAction,
    ...
    const objPropsForTop = {
      ...
      productInCart: objPropsForBottom['productInCart'],
      countProductFromCart: self.countProductFromCart,
      btnCartAction: objPropsForBottom['btnCartAction'],
      changeCountFromCart: self.changeCountFromCart,
      productInComparison: objPropsForBottom['productInComparison'],
      btnComparisonAction: objPropsForBottom['btnComparisonAction'],
      getTitle: objPropsForBottom['getTitle'],
      getLink: objPropsForBottom['getLink'],
      name: self.$props.name,
      image: self.$props.image
    }
    const objPropsForMiddle = {
      onlyCart: objPropsForTop['onlyCart'],
      shortInfo: self.$props.shortInfo,
      calcInfo: self.calcInfo,
      getLink: objPropsForBottom['getLink'],
      getTitle: objPropsForBottom['getTitle']
    }
    return h('div', { class: mainClass }, [
      h('div', { class: classList }, [
        activeIndicator,
        h(ProductCardTop, { props: objPropsForTop }),
        h(ProductCardMiddle, { props: objPropsForMiddle }),
        bottomElement
      ])
    ])
  }
